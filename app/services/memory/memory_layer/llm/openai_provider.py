...
from services.memory.memory_layer.llm.protocol import LLMProvider, LLMError
...
class OpenAIProvider(LLMProvider):
    ...
    def __init__(
        self,
        model: str | None = None,  # None = use local model (no specific model name needed)
        api_key: str | None = None,
        base_url: str | None = None,
        temperature: float = 0.3,
        max_tokens: int | None = 8192,  # Reduced for local models
        ...
        **kwargs,
        ...
        """
        # Use local model by default, or specified model for cloud APIs
        self.model = model or os.getenv("LLM_MODEL") or "local-model"
        self.temperature = temperature
        self.max_tokens = max_tokens
        self.enable_stats = enable_stats  # New

        # Use local VLM by default, fallback to OpenRouter if configured
        # Priority: explicit param > LLM_BASE_URL env > LOCAL_LLM_URL env > local default
        self.api_key = api_key or os.getenv("LLM_API_KEY") or os.getenv("OPENROUTER_API_KEY") or "not-needed"
        self.base_url = base_url or os.getenv("LLM_BASE_URL") or os.getenv("LOCAL_LLM_URL") or "http://127.0.0.1:8007/v1"
    ...
    async def generate(
        ...
        # Prepare request data
        is_azure = "openai.azure.com" in (self.base_url or "")
        data = {
            "model": self.model,
        ...
        if not is_azure and openrouter_provider is not None:
        ...
        for retry_num in range(max_retries):
            try:
                ...
                async with aiohttp.ClientSession(timeout=timeout) as session:
                    url = self.base_url
                    if "openai.azure.com" not in url and not url.endswith("/chat/completions"):
