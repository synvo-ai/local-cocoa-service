...
def get_timezone() -> datetime.tzinfo:
    ...
    try:
        ...
        logger.warning(
            "[DateTimeUtils] Timezone '%s' not found (%s), falling back to UTC",
            tz,
            str(e),
        )
        return datetime.timezone.utc
...
def get_now_with_timezone(tz: datetime.tzinfo = None) -> datetime.datetime:
    """Get current time with local timezone."""
    return datetime.datetime.now(tz=tz or timezone)


def to_timezone(dt: datetime.datetime, tz: datetime.tzinfo = None) -> datetime.datetime:
...
def _parse_datetime_core(
    time_value, target_timezone: datetime.tzinfo = None
) -> datetime.datetime:
    """
...
def from_iso_format(
    create_time, target_timezone: datetime.tzinfo = None, strict: bool = False
) -> datetime.datetime:
    """
