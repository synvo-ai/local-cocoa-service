...
5. build command:
python scripts/build.py --root_dir ROOT_DIR --platform {win,linux,mac} --mode {dev,prod} --no-cache {true, false} --output_dir OUTPUT_DIR

Example:
python scripts/build.py --root_dir . --platform mac --mode dev --output_dir dist
