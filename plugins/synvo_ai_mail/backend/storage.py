...
class EmailMixin(StorageBase):
    ...
    def _row_to_email_account(row: sqlite3.Row) -> EmailAccount:
        return EmailAccount(
            ...
            last_synced_at=dt.datetime.fromisoformat(row["last_synced_at"].replace('Z', '+00:00')) if row["last_synced_at"] else None,
    ...
    def _row_to_email_message(row: sqlite3.Row) -> EmailMessageRecord:
        ...
        return EmailMessageRecord(
            ...
            memory_built_at=dt.datetime.fromisoformat(row["memory_built_at"].replace('Z', '+00:00')) if "memory_built_at" in keys and row["memory_built_at"] else None,
        )
